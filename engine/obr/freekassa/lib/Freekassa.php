...
class Freekassa
{
    ...
    private function GoTofreekassa($request)
    {
        ...
        $o = 'BLACK-COIN';
        $s = md5(implode(':', array($m, $oa, Config::SECRET_KEY_1, $currency, $o)));
		$us_account = $request['account'];

        header("Location: https://pay.freekassa.ru/?m={$m}&oa={$oa}&currency={$currency}&o={$o}&s={$s}&us_account={$us_account}");
        return;
    }
